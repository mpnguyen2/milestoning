...
function [l_hit, r_hit] =shootTraj(init, x, y, ...
        V_arr, sigma, vert_dist, ms_dist, N, num_traj, max_step)
...
while (i < max_step) && ~isempty(ind)
    ...
    pts(:,ind) = pts(:,ind) + V_vec(:,:) * Dt + ...
        sigma * normrnd(0, sqrt(Dt), 2, length(ind));
